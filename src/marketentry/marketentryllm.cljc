(ns marketentry.marketentryllm
  ...
  the Republic of Cuba (CUB) public-sector market-entry compliance
  actor.
  ...
  cited), never a committed record or a real MINCEX filing / Registro
  Mercantil inscription. Every output is censored downstream by
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world MINCEX filing / Registro
  Mercantil inscription request."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けMINCEX提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "fdi-sector=" (:fdi-sector e)
                        " fdi-modality=" (:fdi-modality e)
                        " claimed-approval-authority=" (:claimed-approval-authority e)
                        " onat-record-verified?=" (:onat-record-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-onat-record? e))
                              (:onat-record-verified? e)))
                   0.9 0.3)}))
