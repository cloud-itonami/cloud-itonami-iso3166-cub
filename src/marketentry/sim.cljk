...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (CUB, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up approval-authority-mismatch) ==")
    ...
    (println "== filing/submit eng-4 (approval-authority-mismatch -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up onat-record-unverified) ==")
    ...
    (println "== filing/submit eng-5 (onat-record-unverified -> HARD hold) ==")
