(ns marketentry.store
  "SSoT for the Republic of Cuba (CUB) market-entry compliance actor,
...
(defn demo-data
  ...
  `:fdi-sector` / `:fdi-modality` / `:claimed-approval-authority` are
  ground truth for the `approval-authority-mismatch` flagship check
  (Ley 118 Art.21 Consejo de Estado / Consejo de Ministros tiers);
  `:requires-onat-record?` / `:onat-record-verified?` are ground truth
  for the conditional ONAT NIT tax-record check."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Habana Compañía Mixta SA" :portal "MINCEX / Registro Mercantil"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :fdi-sector :manufacturing :fdi-modality :empresa-mixta
             :claimed-approval-authority :consejo-de-ministros
             :requires-onat-record? true :onat-record-verified? true
             :drafted? false :submitted? false
             :jurisdiction "CUB" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "MINCEX / Registro Mercantil"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :fdi-sector :manufacturing :fdi-modality :empresa-mixta
             :claimed-approval-authority :consejo-de-ministros
    ...
    "eng-3" {:id "eng-3" :operator "Varadero Servicios SA" :portal "MINCEX / Registro Mercantil"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "CUB" :status :intake}
    "eng-4" {:id "eng-4" :operator "Mariel Energía Renovable EAIC" :portal "MINCEX / Registro Mercantil"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             ...
             :drafted? false :submitted? false
             :jurisdiction "CUB" :status :intake}
    "eng-5" {:id "eng-5" :operator "Cienfuegos Logística SA" :portal "MINCEX / Registro Mercantil"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :fdi-sector :manufacturing :fdi-modality :empresa-mixta
             :claimed-approval-authority :consejo-de-ministros
             :requires-onat-record? true :onat-record-verified? false
             :drafted? false :submitted? false
             :jurisdiction "CUB" :status :intake}}})
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               fdi-sector fdi-modality claimed-approval-authority
                               requires-onat-record? onat-record-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    fdi-modality                          (assoc :engagement/fdi-modality fdi-modality)
    claimed-approval-authority            (assoc :engagement/claimed-approval-authority claimed-approval-authority)
    (some? requires-onat-record?)         (assoc :engagement/requires-onat-record? requires-onat-record?)
    (some? onat-record-verified?)         (assoc :engagement/onat-record-verified? onat-record-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/fdi-sector :engagement/fdi-modality :engagement/claimed-approval-authority
   :engagement/requires-onat-record? :engagement/onat-record-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :fdi-sector (:engagement/fdi-sector m) :fdi-modality (:engagement/fdi-modality m)
     :claimed-approval-authority (:engagement/claimed-approval-authority m)
     :requires-onat-record? (boolean (:engagement/requires-onat-record? m))
     :onat-record-verified? (boolean (:engagement/onat-record-verified? m))
...
(defn kotobase-store
  ...
  [{:keys [db-api seq-fn]}]
  (->KotobaseStore (persist/store {:db-api db-api :actor "marketentry-cub"})
