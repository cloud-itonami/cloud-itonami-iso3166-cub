  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Habana Compañía Mixta SA"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Habana Compañía Mixta SA" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest approval-authority-mismatch-is-held-and-unoverridable
  (testing "declared :claimed-approval-authority does not match the independently recomputed Ley 118 Art.21 tier -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:approval-authority-mismatch} (-> (store/ledger db) last :basis)))
...
(deftest onat-record-unverified-is-held-and-unoverridable
  (testing "unverified ONAT NIT tax record when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:onat-record-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
